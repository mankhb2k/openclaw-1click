  ...
  return html`
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        <div>
          <div class="card-title">${t("nodes.list.title")}</div>
          <div class="card-sub">${t("nodes.list.subtitle")}</div>
        </div>
        <button class="btn" ?disabled=${props.loading} @click=${props.onRefresh}>
          ${props.loading ? t("nodes.common.loading") : t("common.refresh")}
        </button>
      </div>
      <div class="list" style="margin-top: 16px;">
        ${
          props.nodes.length === 0
            ? html`
                <div class="muted">${t("nodes.list.empty")}</div>
              `
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <button class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
          ${props.devicesLoading ? t("nodes.common.loading") : t("common.refresh")}
        </button>
      </div>
      ${
        props.devicesError
      ...
      <div class="list" style="margin-top: 16px;">
        ...
        ${
          pending.length === 0 && paired.length === 0
            ? html`
                <div class="muted">${t("nodes.devices.empty")}</div>
              `
            : nothing
        }
      </div>
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="muted" style="margin-top: 6px;">
          ${t("nodes.devices.roleLabel")}: ${roleValue} · ${t("nodes.devices.scopesLabel")}: ${scopesValue} ·
      ...
      <div class="list-main">
        ...
        ${
          tokens.length === 0
            ? html`
                <div class="muted" style="margin-top: 6px;">
                  ${t("nodes.devices.tokensLabel")}: ${t("nodes.devices.none")}
                </div>
              `
            : html`
              <div class="muted" style="margin-top: 10px;">${t("nodes.devices.tokensLabel")}</div>
    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; align-items: center;">
        ...
        <button
          ...
          @click=${state.onSave}
        >
          ${state.configSaving ? t("nodes.common.saving") : t("nodes.common.save")}
        </button>
      </div>
        ...
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            ...
            <button class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
              ${state.configLoading ? t("nodes.common.loading") : t("nodes.binding.loadConfig")}
            </button>
          </div>`
          : html`
            <div class="list" style="margin-top: 16px;">
              <div class="list-item">
                ...
                <div class="list-meta">
                  <label class="field">
                    <span>${t("nodes.binding.nodeField")}</span>
                    ${renderUiSelect({
                      value: defaultValue,
                      disabled: state.disabled || !supportsBinding,
                      options: [
                        { value: "", label: t("nodes.binding.anyNode") },
                  ...
                  ${
                    !supportsBinding
                      ? html`
                          <div class="muted">${t("nodes.binding.noRunnableNodes")}</div>
                        `
                      : nothing
                  }
                </div>
  ...
  return html`
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="list-sub">
          ${agent.isDefault ? t("nodes.binding.defaultAgent") : t("agents.toolbarLabel").toLowerCase()} ·
          ${
      ...
      <div class="list-meta">
        <label class="field">
          <span>${t("nodes.binding.bindingField")}</span>
          ${renderUiSelect({
            value: bindingValue,
            disabled: state.disabled || !supportsBinding,
            options: [
              { value: "__default__", label: t("nodes.binding.useDefault") },
              ...state.nodes.map((node) => ({ value: node.id, label: node.label })),
            ],
            onChange: (next) => {
              const value = next.trim();
...
function renderNode(node: Record<string, unknown>) {
  ...
  const title =
    ...
    (typeof node.nodeId === "string" ? node.nodeId : t("nodes.list.unknown"));
    ...
    <div class="list-item">
      <div class="list-main">
        ...
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${paired ? t("nodes.list.paired") : t("nodes.list.unpaired")}</span>
          <span class="chip ${connected ? "chip-ok" : "chip-warn"}">
            ${connected ? t("nodes.list.connected") : t("common.offline")}
          </span>
