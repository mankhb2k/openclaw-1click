...
import { formatChannelExtraValue, resolveChannelConfigValue } from "./channel-config-extras";
...
export function renderChannelConfigForm(props: ChannelConfigFormProps) {
  ...
  if (!normalized) {
    return html`
      <div class="callout danger">${t("channels.config.schemaUnavailableRaw")}</div>
    `;
  }
  const node = resolveSchemaNode(normalized, ["channels", props.channelId]);
  if (!node) {
    return html`
      <div class="callout danger">${t("channels.config.schemaUnavailableChannel")}</div>
    `;
  }
