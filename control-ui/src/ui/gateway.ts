import { buildDeviceAuthPayload } from "@openclaw/gateway/device-auth.js";
import {
  GATEWAY_CLIENT_CAPS,
...
} from "@openclaw/gateway/protocol/connect-error-details.js";
import {
...
export class GatewayRequestTimeoutError extends Error {
  ...
  constructor(params: { method: string; timeoutMs: number }) {
    super(
      `gateway request timed out (${params.timeoutMs}ms) for ${params.method}`,
    );
...
export class GatewayBrowserClient {
  ...
  private async sendConnect() {
    ...
    const deviceToken =
      selectedAuth.authDeviceToken ?? selectedAuth.resolvedDeviceToken;
    const auth =
  ...
  request<T = unknown>(
    ...
    const timeoutMs = opts?.timeoutMs ?? DEFAULT_GATEWAY_REQUEST_TIMEOUT_MS;
    const p = new Promise<T>((resolve, reject) => {
      const timeoutId = window.setTimeout(() => {
        // If we already received a response, pending entry will be gone and this will no-op.
