...
import { vi as viLocaleMap } from "../locales/vi.ts";
...
describe("i18n", () => {
  ...
  it("defaults to Vietnamese when navigator is not a supported language", async () => {
    vi.resetModules();
    vi.stubGlobal("localStorage", createStorageMock());
    vi.stubGlobal("navigator", { language: "fr-FR" } as Navigator);
    const fresh = await import("../lib/translate.ts");
    expect(fresh.i18n.getLocale()).toBe("vi");
    expect(fresh.t("common.connect")).toBe("Kết nối");
  });
  ...
  it("keeps the version label available in shipped locales", () => {
    expect((viLocaleMap.common as { version?: string }).version).toBeTruthy();
