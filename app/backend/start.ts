  ...
  gatewayChild.on('exit', (code, signal) => {
    ...
    if (code === 1) {
      logLine(
        paths,
        'error',
        'If log shows "Node.js v22.12+ is required": Electron may still ship Node 20 — quit the app, run "npm install" (Electron 35+), or set env OPENCLAW_GATEWAY_NODE to Node 22+ (see resources/README.txt).'
      );
    }
  });
